...
export const getClassroomById = async (dispatch,id) => {
    try {
        // dispatch ({ type: GET_CLASSROOM_BY_REQUEST })
        const res = await axios.get(`/classroom/details/${id}`)
        dispatch({type : GET_CLASSROOM_BY_ID_SUCCESS, payload : res.data})
        return res.data
    } catch (err) {
        // dispatch({type : GET_ALL_CLASSROOMS_FAIL, payload : err})
        console.log(err)
...
export const addTrainee = async (dispatch,data) => {
    try {
        dispatch ({ type: ADD_TRAINEE_REQUEST})
        console.log(data)
        await axios.post('/training/teacher/new',data)
        console.log(data)
        dispatch({type : ADD_TRAINEE_SUCCESS, payload : data})
        // getAllTrainers(dispatch)
    } catch (err) {
        dispatch({type : ADD_TRAINEE_FAIL, payload : err})
    }
}
