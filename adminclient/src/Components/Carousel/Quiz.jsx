...
function Quiz() {
    ...
    const { dispatch, setQuizOpen, state: { quiz}, courseId, quizOpen } = useContext(AppContext)
            ...
            <Box cls="quiz__container">
                <TableHeader Icon={RiLightbulbFlashFill} headerText='Quiz'>
                    <div className="quiz__edit">
                        {!quiz.data[courseId] ? <Button onClick={handleAddQuizModal} style={{ marginRight: '1rem', backgroundColor: '#28a745' }} Icon={CgAddR}>Create Quiz</Button> :
                            <>
                ...
                {quiz.isLoading ?
                    ...
                    </div> : !quiz?.data?.[courseId] ?
                        <p className="warning">Sorry, no quiz data available!</p> :
                    <>
                        <Slider {...settings} ref={sliderRef} className="slider">
                            {QUIZ_QUESTION(quiz.data?.[courseId])?.map((ques, quesIndex) => {
                                return (
