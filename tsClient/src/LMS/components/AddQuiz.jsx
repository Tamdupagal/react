...
function AddQuiz({id, addQuizModal, setAddQuizModal }) {
    ...
    const handleQuizSubmit = () => {
        ...
        if (finalise) {
            if (addQuizModal[id].for === 'quiz') {
                const data = { quiz_details: quizData }
                postNewQuiz(dispatch, data, id, toast, courseId);
            } else if((addQuizModal[id].for === 'question')) {
                postNewQuizQuestion(dispatch, quizData[0], id, toast,courseId)
            } else {
                updateQuizQuestion(dispatch, quizData[0], id, addQuizModal[id].no, toast,courseId);
            }
            setSaveClick(false)
            setQuizData([])
